...
import { ActivityIndicator, StyleSheet, View, Image, Dimensions, TouchableOpacity, ImageBackground, Alert,Platform,ScrollView,I18nManager,AsyncStorage,FlatList } from 'react-native';
...
this.setState({
     ...
     console.log("testArr1",this.state.testArr)
    }
...
console.log("testarr2",this.state.testArr)
    ...
    client.post(`/app/getallproducts?page_number=${this.offset_guest}&categories_id=${this.props.navigation.state.params.itemId}&language_id=${lang}&type=${type}`).then((res) => {
      if(res.data.status==200){
        this.setState({status:200})
      }else{
        this.setState({status:204})        }
...
this.setState({
      ...
      console.log("testarr3",this.state.testArr)
    ...
    console.log("this.state.testArr444",this.state.testArr)
  ...
  static navigationOptions = {
   ...
   async componentWillMount(){
    ...
    const value = await AsyncStorage.getItem('userID');
      ...
      console.log("this.props.navigation.state.params.itemId",this.props.navigation.state.params.itemId)
    console.log("this.offset_defult66",this.offset_defult)
    console.log("lang",lang)
    console.log("client",client)
    this.setState({userID:value})
client.post(`/app/getallproducts?page_number=${this.offset_defult}&categories_id=${this.props.navigation.state.params.itemId
}&language_id=${lang}`).then((res) => {
  console.log("ress2222",res.data.product_data)
...
/>
  ...
  loadMoreData = () => {
    ...
    loadMoreDataDefult = (type) => {
      console.log("typeeeee in load more",type)
      console.log("offfst in load more",this.offset_defult)
      ...
      {
        console.log("hereeeeeeeeeeeeeeeee",this.props.navigation.state.params.itemId)
          client.post(`/app/getallproducts?page_number=${this.offset_defult}&categories_id=${this.props.navigation.state.params.itemId}&language_id=${lang}`).then((res) => {
console.log("res11122223333",res)
            if(res.data.message=='Returned all products.'){
              this.offset_defult = this.offset_defult + 1;
        ...
        client.post(`/app/getallproducts?page_number=${offset_guest_load}&categories_id=${this.props.navigation.state.params.itemId}&language_id=${lang}&type=${type}`).then((res) => {
...
if(this.state.wish.includes(products_id)){
  ...
  client.post(`/app/likeproduct?liked_products_id=${products_id}&liked_customers_id=${this.state.userID}`).then((res) => {
    ...
    handelAddToOrder(item){
      ...
      let test=0;
      if(item.bounce!=null&&item.bounce!=null){
...
color: "#777777"}} >{i18n.t('noRecordFound')}</Text>
  ...
  render() 
  {
   ...
   console.log("this.state.defultArr",this.state.defultArr)
    i18n.fallbacks = true;
    i18n.translations = { ar, en };
...
console.log("this.state.testAtt",this.state.testArr)
