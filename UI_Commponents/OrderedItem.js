...
export const OrderedItem = ({...props}) => {
    ...
    return (
      ...
      >
          ...
          <CardItem style={{backgroundColor:'transparent'}} style={{justifyContent:'space-between',alignItems:'center',flexDirection:'row',height:60}}>
             {/* <Left style={{backgroundColor:'transparent',width:'100%'}}> */}
                <View style={{flexDirection:'row',alignItems:'center'}}>
             <Image source={{uri:item.image}} style={{height: 35, width: 35,resizeMode:'contain'}}/>
          
                 <Text style={[styles.itemText,{marginTop:0,marginBottom:0}]}>{item. products_name}</Text>
                 </View>
                 {/* <View style={{justifyContent:'flex-start',width:100,backgroundColor:'red',alignItems:'flex-end'}}></View> */}
                 <Text style={[styles.itemText,{marginTop:0,marginBottom:0,marginEnd:7}]}>{item.customers_basket_quantity}</Text>
                 <Text style={[styles.itemText,{marginTop:0,marginBottom:0,marginEnd:7}]}>{`${parseFloat(item.final_price).toFixed(2)}JOD`}</Text>
