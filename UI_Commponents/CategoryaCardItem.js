...
export const CategoryaCardItem = ({...props}) => {
    ...
    return (
      ...
      <TouchableOpacity
        onPress={() =>{
          ...
          props.navigate(props.item.id,props.item.image,props.item.name,props.item.total_products)
          }}
      >
      ...
      <Image source={{uri:BaseURL + item.image}} style={{height: '30%', width: '45%', resizeMode:'contain'}}/>
      <View  style={{justifyContent:'center',alignItems:'center',}} >
      <Text   numberOfLines={2}
 style={[styles.itemText,{marginTop:7,marginBottom:0,color:'#707070'}]}>{item.name}</Text>
 </View>
